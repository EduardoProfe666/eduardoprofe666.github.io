            ...
            {mode === "search" ? (
              <>
                <div className="flex items-center gap-3 border-b border-border/60 px-4">
                  ...
                  />
                  <kbd className="hidden shrink-0 rounded-md border border-border/70 px-1.5 py-0.5 text-2xs font-medium text-muted-foreground sm:block">
                    esc
                  </kbd>
                </div>
                ...
                <div ref={listRef} className="max-h-[52vh] overflow-y-auto p-1.5">
                  ...
                  {rows.map(({ item, header }, index) => {
                    ...
                    return (
                      <div key={item.id}>
                        {header && (
                          <p className="px-3 pb-1 pt-3 text-2xs font-semibold uppercase tracking-wider text-muted-foreground/70">
                            {header}
                          </p>
                        )}
                        <button
                        ...
                        >
                          ...
                          {item.hint && (
                            <kbd className="shrink-0 rounded-md border border-border/70 px-1.5 py-0.5 text-2xs font-medium text-muted-foreground">
                              {item.hint}
                            </kbd>
                          )}
                        </button>
              ...
              <div className="p-5">
                    ...
                    >
                      ...
                      <span className="flex shrink-0 items-center gap-1">
                        {row.keys.map((key) => (
                          <kbd
                            key={key}
                            className="min-w-6 rounded-md border border-border/70 bg-background/60 px-1.5 py-0.5 text-center text-2xs font-medium text-muted-foreground"
                          >
                            {key}
                          </kbd>
                        ))}
                ...
                <p className="mt-4 flex items-center justify-center gap-1.5 text-2xs text-muted-foreground">
                  ...
                  {t("keys.close")}
                  <kbd className="rounded-md border border-border/70 px-1 py-0.5 text-2xs">
                    esc
                  </kbd>
                </p>
              </div>
